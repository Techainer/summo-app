...
// ---- wide, dark ----------------------------------------------------------
{
  ...
  const folders = await page.getByRole('navigation', { name: 'Thư mục', exact: true }).getByRole('button').allInnerTexts();
  ...
  await page.getByRole('navigation', { name: 'Màn hình' }).getByRole('button', { name: 'Thống kê' }).click();
  await page.waitForTimeout(600);
  await page.screenshot({ path: `${shots}/shell-analytics.png` });

  // The sidebar collapses on demand and comes back.
  await page.getByRole('navigation', { name: 'Màn hình' }).getByRole('button', { name: 'Thư viện' }).click();
  ...
  try {
    await page.getByRole('navigation', { name: 'Thư mục', exact: true }).waitFor({ state: 'hidden', timeout: 4000 });
  } catch {
