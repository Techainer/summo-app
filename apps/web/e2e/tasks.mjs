...
const context = await browser.newContext({ locale: 'vi-VN', viewport: { width: 1400, height: 900 }, colorScheme: 'dark' });
...
if (await expand.count() === 0) problems.push('the agent task showed no step list');
  ...
  await page.waitForTimeout(300);
  const steps = await page.locator('li').filter({ hasText: /Quét ghi chú|Soạn sự kiện/ }).count();
