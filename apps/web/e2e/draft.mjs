...
const context = await browser.newContext({ locale: 'vi-VN', viewport: { width: 1300, height: 950 }, colorScheme: 'dark' });
...
await page.goto(`${appUrl}?port=${port}&token=${token}#/meetings/${meetingId}`, { waitUntil: 'networkidle' });
...
if (occurrences !== 1) problems.push(`unapproved text appeared ${occurrences} times, expected once`);

if (await page.getByRole('button', { name: 'Xác nhận' }).count() === 0) problems.push('no confirm button');
if (await page.getByRole('button', { name: 'Bỏ' }).count() === 0) problems.push('no discard button');

// Selecting a passage should offer to rewrite that passage.
await page.evaluate(() => {
  const p = [...document.querySelectorAll('p')].find((el) => el.textContent?.includes('Chốt ngân sách quý bốn'));
