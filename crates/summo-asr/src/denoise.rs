...
mod gtcrn {
    ...
    impl Gtcrn {
        ...
        pub fn load(path: &str, threads: u32, name: &str) -> Result<Self> {
            let model = CString::new(path)
                .map_err(|_| Error::Config(format!("model path is not usable as a C string: {path}")))?;
    ...
    impl Denoiser for Gtcrn {
        fn denoise(&mut self, pcm: &[f32]) -> Result<Vec<f32>> {
            ...
            let n = c_int::try_from(pcm.len()).map_err(|_| {
                Error::Other(format!("{} samples is more than one call can carry", pcm.len()))
            })?;
    ...
    mod tests {
        ...
        fn measure_real_time_factor() {
            ...
            for threads in [1u32, 2, 4, 8] {
                ...
                eprintln!("{threads}t: 3 s utterance in {per:.3} s -> rtf {:.4}", per / 3.0);
            }
        }
