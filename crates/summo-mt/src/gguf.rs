//! MiLMMT and anything else in GGUF, through llama.cpp.
//!
//! The quality option of the two runtimes here. A 1B decoder-only translation model is the best
//! translation that fits on a laptop, and llama.cpp is what runs one — but it is a C++ build, and
//! Gemma-based models cannot go much below 650 MB whatever the quantization, because the 262 000
//! token embedding table dominates the file. See [`crate::seq2seq`] for the small option.
